...
app.listen(PORT, () => {
  logger.info(`API RsiCodeQrApp démarrée sur http://localhost:${PORT}`);
...
cron.schedule("0 6 * * *", async () => {
  logger.info("[PLANIFICATEUR] 06h00 — vérification du rapport à envoyer…");
  const resultat = await declencherRapportsProgrammes();
  if (resultat.ok) {
    logger.info("[PLANIFICATEUR] ✓", resultat.raisons.join(" ; "));
  } else if (resultat.raisons.length > 0) {
    logger.info("[PLANIFICATEUR] rien à faire :", resultat.raisons.join(" ; "));
  }
});